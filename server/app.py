...
def bakery_by_id(id):
    ...
    if bakery == None:
        response_body = {"message": f"could not find a bakery with id {id}"}
...
def baked_goods():
    bg = [baked_good.to_dict() for baked_good in BakedGood.query.all()]
    if request.method == "GET":
        return make_response(bg, 200)
    else:
        if request.method == "POST":
            new_bg = BakedGood(
                name=request.form.get("name"),
                price=request.form.get("price"),
                created_at=request.form.get("created_by"),
            ...
            response = make_response(bg_dict, 201)
            return response
...
def baked_goods_by_id(id):
    ...
    if bg == None:
        response_body = {"message": f"could not find a baked good with id {id}"}
        response = make_response(response_body, 404)
        return response
    else:
        if request.method == "GET":
